      ...
      <form @submit.prevent="submitCreate" class="space-y-5">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <select v-model="createForm.customerId" class="input" required>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="label">Sector de Aplicación</label>
          ...
          <div>
            <label class="label">Tipo de Aluminio</label>
        ...
        <div>
          ...
          <textarea v-model="createForm.notes" class="input" rows="2" placeholder="Observaciones o instrucciones especiales..."></textarea>
        </div>

        <!-- Items -->
        <div>
          <div class="flex items-center justify-between mb-2">
            <label class="label mb-0 font-semibold">Artículos *</label>
          ...
          <div class="space-y-2">
            <div
              ...
              :key="idx"
              class="grid grid-cols-12 gap-2 items-center bg-gray-50 dark:bg-slate-800/50 rounded-lg p-2"
            >
              <!-- Product select -->
              ...
              <div class="col-span-1">
                <input v-model.number="item.qty" type="number" min="0.01" step="0.01" class="input text-sm text-center" placeholder="1" />
              </div>
              <!-- Unit price -->
              ...
              <!-- Subtotal -->
              <div class="col-span-1 text-right text-sm font-medium text-slate-700 dark:text-gray-300">
        ...
        <div class="bg-gray-50 dark:bg-slate-800 rounded-xl p-4 text-sm space-y-1.5 border border-gray-200 dark:border-slate-700">
          ...
          <div class="flex justify-between text-gray-600 dark:text-gray-400">
            <span>Descuento</span>
            <span class="text-red-500">-{{ formatCurrency(saleDiscount) }}</span>
          </div>
          <div class="flex justify-between text-gray-600 dark:text-gray-400">
            <span>IVA (16%)</span>
...
async function submitCreate() {
  if (!createForm.customerId || createForm.items.some(i => !i.productId || !i.qty)) {
    toast.warning('Selecciona un cliente y completa todos los artículos')
    return
  }
  saving.value = true
  try {
